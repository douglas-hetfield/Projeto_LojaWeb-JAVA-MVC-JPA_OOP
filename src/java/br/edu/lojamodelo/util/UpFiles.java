package br.edu.lojamodelo.util;
...
public class UpFiles{
    ...
    public boolean send(HttpServletRequest request) throws ServletException, IOException, Exception {
            System.out.println("AQUIII ESTA O CAMINHO: " + SAVE_DIR);
        try {
            //Define a pasta para slavar a foto
            String savePath = SAVE_DIR;   //request.getServletContext().getRealPath("//"+SAVE_DIR);   ESTAVA assim antes, mas voltava null
            System.out.println("AQUIII ESTA O CAMINHO: " + savePath);
            ...
            if (ServletFileUpload.isMultipartContent(request)) {
                ...
                for (FileItem item : multiparts) {
                    ...
                    if (!item.isFormField()) {
                        ...
                        // if (tipo[1].length() <= 4) {
                        fileName[x] = br.edu.lojamodelo.util.Crypt.md5(item.getName()) + "." + tipo[1];
